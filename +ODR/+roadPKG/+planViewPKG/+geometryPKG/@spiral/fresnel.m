...
function [cca,ssa] =  fresnel(obj,xxa )
    %FRESNEL Approximate fresnel integral
    %
    %----------------------------------------------------------------------
    ...
    if ( x2 < 2.5625 )
        t = x2 * x2;
        ss = x * x2 * obj.polevl (t, sn) / obj.p1evl (t, sd);
        cc = x * obj.polevl (t, cn) / obj.polevl (t, cd);
    elseif ( x > 36974.0 )
        cc = 0.5;
        ss = 0.5;
    else
        ...
        u = 1.0 / (t * t);
        t = 1.0 / t;
        f = 1.0 - u * obj.polevl (u, fn) / obj.p1evl(u, fd);
        g = t * obj.polevl (u, gn) / obj.p1evl (u, gd);
